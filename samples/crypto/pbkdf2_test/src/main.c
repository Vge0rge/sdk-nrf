...
/*
static void DeriveVerifier(ByteSpan salt, uint32_t password, uint32_t iterations, MutableByteSpan &verifier)
{
  psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
  psa_key_derivation_operation_t operation = PSA_KEY_DERIVATION_OPERATION_INIT;
  psa_key_id_t passwordKey = PSA_KEY_ID_NULL;
  psa_key_id_t spakeKey = PSA_KEY_ID_NULL;
  size_t verifierLen = verifier.size();

  psa_crypto_init();

  // Prepare password key
  uint8_t passwordBytes[sizeof(uint32_t)];
  Encoding::LittleEndian::Put32(passwordBytes, password);

  psa_set_key_usage_flags(&attributes, PSA_KEY_USAGE_DERIVE);
  psa_set_key_algorithm(&attributes, PSA_ALG_PBKDF2_HMAC(PSA_ALG_SHA_256));
  psa_set_key_type(&attributes, PSA_KEY_TYPE_PASSWORD);

  psa_status_t status = psa_import_key(&attributes, passwordBytes, sizeof(passwordBytes), &passwordKey);
  VerifyOrExit(status == PSA_SUCCESS, LogPsaError(status));
  ...
  status = psa_export_public_key(spakeKey, verifier.data(), verifier.size(), &verifierLen);
